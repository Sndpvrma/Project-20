...
class HtmlUtility:
    ...
    def get_list_from_list(name: str, selected_val: str, list_data: list[str]) -> str:
        ...
        Returns:
            An HTML <select> string with a default --Select-- option.
        """
        sb = [f"<select class='form-control' style='width:100%' name='{name}'>"]
        sb.append("<option value='0'>--Select--</option>")
    ...
    @staticmethod
    def get_list_from_beans(name: str, selected_val: int, bean_list: list[DropdownItem], onchange: str = "") -> str:
        """
        ...
        sb = [f"<select class='form-control' style='width:100%' name='{name}' onchange='{onchange}'>"]
        sb.append("<option value='0'>--Select--</option>")
    ...
    @staticmethod
    def get_options_from_beans(selected_val: int, bean_list) -> str:
        """
        Generate only HTML <option> tags from bean list (NO <select> wrapper)
        """
