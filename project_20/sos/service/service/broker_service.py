...
class BrokerService(BaseService):
    ...
    def search(self, params):
        ...
        value = params.get("broker_name", None)
        if DataValidator.is_not_null(value):
            query = query.filter(broker_name__istartswith=value.strip())

        value = params.get("contact_number", None)
        if DataValidator.is_not_null(value):
            query = query.filter(contact_number__istartswith=value.strip())

        value = params.get("company", None)
        if DataValidator.is_not_null(value):
            query = query.filter(company__istartswith=value.strip())
